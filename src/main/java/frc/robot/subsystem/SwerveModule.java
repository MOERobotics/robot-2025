...
public class SwerveModule extends MOESubsystem<SwerveModuleInputsAutoLogged> implements SwerveModuleControl {
    ...
    public SwerveModuleState getModuleState() {
        return new SwerveModuleState(
                InchesPerSecond.of(driveMotor.getEncoder().getVelocity() * (3.76 * Math.PI / 6.75) / 60).in(MetersPerSecond),
    ...
    public SwerveModulePosition getModulePosition() {
        SwerveModulePosition position = new SwerveModulePosition(
            Inches.of(driveMotor.getEncoder().getPosition() * (3.76 * Math.PI / 6.75)).in(Meters),
            new Rotation2d(getHeading())
        );
        return position;
    }
